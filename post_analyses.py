...
def bimodal_fit(x, y, sd):
	...
	params,_ = curve_fit( gauss, x, y, 
		bounds=((20.0,  0.0,     0.0),  (60.0, 100.0, 100.0)), 
		p0=(30.0,  5.0,  20.0),
		maxfev=10000000,
	) 
...
def select_ids_by_position(positions, sheet_ids, radius=[0,0], box=[], reverse=False, origin=[[0.0],[0.0],[0.0]]):
	selected_ids = []
	distances = []
	min_radius = radius[0]
	max_radius = radius[1]
...
def get_per_neuron_window_spike_count(
    datastore, sheet, stimulus, stimulus_parameter, start, stop, window=50.0, neurons=None, nulltime=False
):
    if neurons is None:
        neurons = []
    ...
    stimuli = list(dic.values())[0][0]  # Fixed for Python 3
    means = numpy.array(list(dic.values())[0][1])  # Fixed for Python 3
    ...
    variances = numpy.array(list(dic.values())[0][1])  # Fixed for Python 3
...
def get_per_neuron_spike_count( datastore, stimulus, sheet, start, end, stimulus_parameter, bin=10.0, neurons=None, spikecount=True ):
	if neurons is None:
		neurons = []
...
def size_tuning_comparison( sheet, folder_full, folder_inactive, stimulus, parameter, box=None, radius=None, csvfile=None, plotAll=False ):
	...
	changing_idxs = list(zip(changed_units[0], changed_units[1]))
...
def activity_ratio( sheet, folder, stimulus, stimulus_parameter, arborization_diameter=10, addon="", color="black" ):
	...
	plt.bar(x, sorted(list(above_spontaneous_perc.values())), align='center', width=0.8, color=color)
...
def response_barplot( sheet, folder, stimulus, parameter, num_stim=10, max_stim=1., box=None, radius=None, xlabel="", data="", data_marker="D" ):
	...
	plt.hist(dt, num_stim, density=True, histtype='bar', stacked=True, range=(0.0, max_stim))
...
def LHI( sheet, folder, stimulus, parameter, num_stim=2, addon="" ):
	...
	def dLHI(sheet, positions, sheet_indexes, vms, ids, ids_positions, sigma):
		...
		for vm, i, x in zip(vms, ids, ids_positions):
			...
			# print "dLHI",dLHI[i]
		print("dLHI extremes",numpy.mean(list(dLHI.values())),numpy.std(list(dLHI.values())),min(dLHI.values()), max(dLHI.values()))
...
def SynergyIndex_spikes( sheet, folder, stimulus, parameter, preferred=True, num_stim=2, sigma=.280, bins=102, radius=None, addon="" ):
	...
	spike_ids = param_filter_query(data_store, sheet_name=sheet, st_name=stimulus).get_segments()[0].get_stored_spike_train_ids()
	if spike_ids is None or len(spike_ids)<1:
	...
	for s in segs:
		...
		trials_mean_SI.append( numpy.mean(list(SI.values()), axis=0) )
		trials_stdev_SI.append( numpy.std(list(SI.values()), axis=0) )
...
def SingleTrialSynergyIndex_spikes( sheet, folder, stimulus, parameter, preferred=True, num_stim=2, sigma=.280, bins=102, radius=None, addon="" ):
	...
	for s in segs:
		...
		trials_pop_mean_SI.append( numpy.mean(list(SI.values()), axis=0) ) # mean SI of all units over time for this trial
		trials_pop_stdev_SI.append( numpy.std(list(SI.values()), axis=0) ) # its std
