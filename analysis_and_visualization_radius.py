...
def perform_analysis_and_visualization(data_store, atype='contrast', withPGN=False, withV1=False):
    ...
    print(f"spike_Xon_ids: {spike_Xon_ids}")
    print(f"spike_Xoff_ids: {spike_Xoff_ids}")
    ...
    for i in Xon_sheet_ids:
        a = np.array((position_Xon[0][i], position_Xon[1][i], position_Xon[2][i]))
        l = np.linalg.norm(a - 0.0)
        if 0.75 < l < 2.0:
            nonoverlap_Xon_ids.append(i[0])
                #print a
    for i in Xoff_sheet_ids:
        a = np.array((position_Xoff[0][i], position_Xoff[1][i], position_Xoff[2][i]))
        l = np.linalg.norm(a - 0.0)
        if 0.75 < l < 2.0:
    ...
    print(f"nonoverlap_Xon_ids: {nonoverlap_Xon_ids}")
    print(f"nonoverlap_Xoff_ids: {nonoverlap_Xoff_ids}")
